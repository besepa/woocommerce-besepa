	...
	function process( CheckoutData $checkoutData, $subscription_payment = false )
	{
        ...
        if( $customer && $checkoutData->bankAccount )
        {
            ...
            if ($debit = $this->createDebit($checkoutData, $customer)) {
                ...
                update_post_meta($checkoutData->order->id, "besepa_bank_account_id", $checkoutData->bankAccount->id, true);
                update_post_meta($checkoutData->order->id, "besepa_customer_id", $customer->id, true);
                update_post_meta($checkoutData->order->id, "besepa_debit_id", $debit->id, true);
